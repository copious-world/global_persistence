...
class CategoricalPersistenceManager extends PersistenceManager {
  ...
  constructor(persistence,message_relays) {
      ...
      this.repo_link_conf = false
  }

  //
  async initialize(conf,db) {
      this.conf = conf.repo_bridge_and_com ? conf.repo_bridge_and_com : conf
      if ( this.conf === undefined ) {
        console.warn("the repo_bridge is not defined for global categories in `global_persistence` in method 'initialize'")
        return
      }
      if ( this.conf.media_handler ) {    // alternate globally installed media hanlder
        const MediaHandlerClass = require(conf.media_handler)
        this.media_handler = new MediaHandlerClass(this.conf)
      } else {
        this.media_handler = new MediaHandler(this.conf.media_handler_conf) // 
      }
      super.initialize(this.conf,db)    // db is not in use this version...
      await this.setup_relays_and_crypto()
  }

  /**
   * setup_relays_and_crypto
   * 
   * called by initialize
   * 
   * @returns promise|undefined
   */
  async setup_relays_and_crypto() {
      let conf = this.conf
      conf = conf ? conf.relays_and_crypto : false
      if ( this.conf === undefined ) {
   ...
   * await_ready
   * 
   * called by setup_relays_and_crypto
   * 
   * @returns promise|undefined
   */
  async await_ready() {
  ...
  async startup() {
      ...
      try {
        // order is important
        // media_handler gets to use the link manager (for repository bridge)
        this.media_handler.set_link_manager(this.repo_link_conf,this.link_manager)
        // init_repository is next
        await this.media_handler.init_repository(this.conf.media_handler_conf)
      } catch (err) {
          console.error(err)
      }
      //
