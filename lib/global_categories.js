...
function light_meta(data,finally_tracking) {
  return {}
}
  ...
  async new_entry(data,update) {
    ...
    if ( data.media && data.media.source ) {
      if ( (data.media_type !== 'image') && this.media_handler ) {
        let media = data.media.source                     // media is not poster
    ...
    return light_meta(data,finally_tracking)
  }
